...
void save_history() {
    ...
    if (home == NULL) { // Verifica si la variable de entorno HOME está definida
        perror("getenv"); // Imprime un mensaje de error utilizando perror si la variable no está definida
        return; // Retorna si la variable no está definida
    }
    ...
    if (file == NULL) { // Verifica si se pudo abrir el archivo
        perror("fopen"); // Imprime un mensaje de error utilizando perror si no se pudo abrir el archivo
        return; // Retorna si no se pudo abrir el archivo
    }
